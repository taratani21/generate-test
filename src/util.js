...
class Util {
  ...
  static isFunctionExpr(node) {
    return (
      node.arguments && node.arguments[0] && node.arguments[0].type.match(/FunctionExpression/)
    );
  }
  ...
  static getAngularType(typescript) {
    return typescript.match(/^\s*@Component\s*\(/m)
      ? 'component' /* eslint-disable */
      : typescript.match(/^\s*@Directive\s*\(/m)
      ? 'directive'
      : typescript.match(/^\s*@Injectable\s*\(/m)
      ? 'service'
      : typescript.match(/^\s*@Pipe\s*\(/m)
      ? 'pipe'
      : 'obj'; /* eslint-enable */
  }

  static getClassName(tsPath) {
    return basename(tsPath)
  ...
  static objToJS(obj, level = 1) {
    ...
    const firstKey =
      typeof obj === 'object' && Object.keys(obj).filter((k) => k !== 'undefined')[0];
    if (typeof obj === 'function') {
      const objRet = obj();
      ...
      for (var key in obj) {
        ...
        if (typeof obj[key] === 'object' && !obj1stKey) {
          // is empty obj, e.g. {}
  ...
  static merge(source, target) {
    ...
    if (firstKey && !target[firstKey]) {
      ...
      if (typeof sourceFuncRet === 'object') {
        ...
        const isTarget0EmptyObj =
          targetFuncRet[0] && // e.g.,  { 0 : {} }
          Object.keys(targetFuncRet[0]).length === 0 &&
          targetFuncRet[0].constructor === Object;
        const isTarget0Exists =
          targetFuncRet[0] && // e.g.,  { 0 : {foo;bar} }
          Object.keys(targetFuncRet[0]).length !== 0 &&
          targetFuncRet[0].constructor === Object;
  ...
  static getFuncParamCodes(code) {
    ...
    paramNames.forEach((paramName) => {
      const paramNameMatchRE = new RegExp(
        `[^a-z]${paramName}(\\.[^\\s\\;\\)\\\+\\-\\]\\}\\,]+)+`,
        'img',
      );
  ...
  static getFuncMockJS(mockData, thisNameInput = 'component', sources) {
    ...
    Object.entries(mockData.globals).forEach(([key1, value]) => {
      // window, document
  ...
  static getFuncParamJS(params) {
    ...
    Object.entries(params).forEach(([key2, value2]) => {
      const value21stKey =
        typeof value2 === 'object' && Object.keys(value2).filter((k) => k !== 'undefined')[0];
